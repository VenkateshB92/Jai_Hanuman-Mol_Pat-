...
def draw_graph(data, explanation, word, word1, word_p):
    edges = []
    ...
    print("Actual N Mask",node_mask)
    print("Actual Node Mask Average:",np.mean(node_mask))
    ...
    nx.draw(g, pos, labels=labels, with_labels=True, node_color=node_colors, edge_color=edge_colors,
            node_size=50, font_color='black', ax=ax, width=3)
    ...
    nx.draw(g, pos, labels=labels, with_labels=True, node_size=50, ax=ax, width=2)
    plt.savefig(word1, dpi=300)
    plt.close

    #Modified
    node_mask = explanation.node_mask.squeeze().tolist()
    node_mask_values = {i: np.random().rand for i, value in enumerate(node_mask)}
    nx.set_node_attributes(g, node_mask_values, 'value')
    edge_mask = explanation.edge_mask.numpy()
    # Calculate average values
    print("Predicted Mask",node_mask)
    node_avg = np.mean(node_mask)
    print(f"Pred Node Mask Avg: {node_avg}")
    ...
    filtered_nodes = [node for node, value in enumerate(node_mask) if value >= node_avg] or None
    #filtered_edges = [(u, v) for u, v in g.edges() if g[u][v]['weight'] >= edge_avg and u in filtered_nodes and v in filtered_nodes]
   
    # Create filtered subgraph
    #g_filtered = nx.Graph(g)

     # Create filtered subgraph 
    g_filtered = nx.Graph()  
    #g_filtered.add_edges_from(filtered_edges)

    # Add only filtered nodes that are part of the filtered edges
    ...
    node_colors1 = [node_cmap(node_norm(value)) for value in filtered_nodes]
    #node_colors1 = [node_cmap(node_norm(node_mask[node])) for node in filtered_nodes]
    #node_colors = [node_cmap(node_norm(filtered_nodes)) for filtered_nodes in node_values]
    edge_colors1 = [edge_cmap(edge_norm(weight)) for weight in edge_values]
    #edge_colors1 = [edge_cmap(edge_norm(g[u][v]['weight'])) for weight in filtered_edges]

    pos1 = nx.spring_layout(g_filtered)

    fig, ax = plt.subplots()
    nx.draw(g_filtered, pos1, labels=labels, with_labels=True, node_color=node_colors1, edge_color=edge_colors1,
            node_size=50, font_color='black', ax=ax, width=3)
    
    plt.savefig(word_p, dpi=300)
    plt.show()
    plt.close

    """node_colors = [node_mask.get(node, 0.0) for node in graph.nodes()]
    nx.draw(graph, pos, with_labels=True, node_color=node_colors, cmap=plt.cm.viridis, node_size=500, font_size=10)
...
def predicting(model, device, loader):
    ...
    for data in loader:
        ...
        try:
            draw_graph(data, explanation, word, word1,word_p)
        except:
            continue
