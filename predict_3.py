...
def draw_graph(data, explanation, word, word1):
    ...
    # print("Original Nodes",g.nodes + "and Total No. of Nodes",g.number_of_nodes())
    # print("Original Edges",g.edges + "Total No. of Edges",g.graph)
    #nx.draw(g)
    nodeg_norm = plt.Normalize(vmin=0, vmax=1)
    edgeg_norm = plt.Normalize(vmin=0, vmax=1)
    
    nodeg_cmap = plt.cm.Brown
    edgeg_cmap = plt.cm.Pink
        ## -->original node_colors and edge_colors
        ...
        # edge_colors = [edge_cmap(edge_norm(weight)) for weight in edge_values] 
         ...
         # Node color based on the filtered node mask
        # node_colors = [node_cmap(node_norm(value)) if value > 0 else (1, 1, 1, 0) for value in node_values]  # Use transparent for below-average
        # edge_colors = [edge_cmap(edge_norm(weight)) for weight in edge_values]
    nodeg_colors = [nodeg_cmap(nodeg_norm(value)) for value in node_values]
    edgeg_colors = [edgeg_cmap(edge_norm(g[u][v]['weight'])) for u, v in g.edges()]

    pos = nx.spring_layout(g)

    fig, ax = plt.subplots()
    nx.draw(g, pos, labels=labels, with_labels=True, node_color=nodeg_colors, edge_color=edgeg_colors,
    node_size=150, font_color='black', ax=ax, width=3)
    plt.savefig(f"Ground_Truth_{i}.png")

    node_mask = explanation.node_mask.squeeze().tolist()
    ###############################
    #code added 
    print(f"Actual Node_mask_{i}: {node_mask}")  # Display the value of i along with node_mask
    avg = numpy.average(node_mask)
    print(f"Average of Node_mask_{i}: {avg}")  # Display the value of i along with average

    # Filter node_mask values: Keep only those >= average, set others to 0
    filtered_node_mask_values = {i: (value if value >= avg else None) for i, value in enumerate(node_mask)}
    print("Predicted Node Mask Values:", filtered_node_mask_values)
...
################ upto here code added
    nx.set_node_attributes(g, filtered_node_mask_values, 'value')
    ...
    for i, (u, v) in enumerate(g.edges()):
        ...
        # Update node_values array based on the filtered mask
        #node_values = np.array([data['value'] for _, data in g.nodes(data=True)])
    
        # Filter out nodes with values below average (those with value == None)
        ...
        # edge_colors = [edge_cmap(edge_norm(weight)) for weight in edge_values]
        node_colors = [node_cmap(node_norm(value)) for value in node_values]
        edge_colors = [edge_cmap(edge_norm(g_filtered[u][v]['weight'])) for u, v in g_filtered.edges()]
    ...
    # cbar_edge = plt.colorbar(edge_sm, ax=ax, orientation='vertical', label='Edge Value')
        ...
        nx.draw(g_filtered, pos, labels=labels, with_labels=True, node_size=150, ax=ax, width=2)
