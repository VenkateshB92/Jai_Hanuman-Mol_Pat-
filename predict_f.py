...
def draw_graph(data, explanation, word, word1, word_filtered):
    ...
    # Changing the Edge_Mask to Important_Edge_Mask 
    print("Actual edge_mask",edge_values)
    average_edge_value = np.mean(edge_values)
    print("Average of edge_mask",average_edge_value)

    for i,item in enumerate(edge_values):  #item -> weight
        if item < average_edge_value:
            edge_values[i] = 0.0
    Imp_edge_values = edge_values
    print("Important edge_mask",Imp_edge_values)

    for i, (u, v) in enumerate(g.edges()):
        g[u][v]['weight'] = Imp_edge_values[i]
    
    #nx.set_edge_attributes(g, Imp_edge_values, 'weight')
    
    #g_filtered = g.subgraph(Imp_node_mask_values).copy()
    ...
    node_colors = [node_cmap(node_norm(value)) for value in node_values] 
    #edge_colors = [edge_cmap(edge_norm(g[u][v]['weight'])) for (u, v) in edge_values]  #g.edges()/edge_values
    edge_colors = [edge_cmap(edge_norm(weight)) for weight in edge_values]  #g.edges()/edge_values
    pos = nx.spring_layout(g)
...
def predicting(model, device, loader):
    ...
    for data in loader:
            ...
            word_filtered = "filt_graph" + str(i) + ".png"
            try:
